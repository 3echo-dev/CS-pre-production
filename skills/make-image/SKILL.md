...
3. Put the quote to the person on the board and in chat as one question (`board-sync.js ask ... --item storyboard`), panel count, credits, style preamble. **Wait for an explicit yes.** The spend guard refuses without it.
...
`stage.js` at both ends: `stage-1-creative --substep "Storyboard: quoting" --agents "orchestrator:working"` for steps 1 to 4, then `--substep "Storyboard: panels" --agents "image-maker:working"`.
...
3. Never request legible text, headlines, logos or UI screenshots.
...
5. A thumbnail is review quality only; say so.
...
8. A real place from the client assets looks like the client's picture, not like a guess.
9. `assetIds` takes 16 references per job. `aspectRatio` is one of `1:1 2:3 3:2 3:4 4:3 9:16 16:9 21:9`; a film board is usually `16:9`, from the brief, never assumed.
