...
2. Quote `count x 1 credit` for items not `validated`. Over `credit_ceiling_per_job` (`CONFIG.md`) stop and ask what to cut.
...
`storyboard/v{n}/P{id}.png` per panel, `P{id}-r{k}.png` per archived regeneration, `contact-sheet.png` rebuilt on every change, one board `panels/P{id}` document per panel. The manifest updated in place with `status`, `file`, `assetIds` and `credits`. `status.md` carries the tally.
