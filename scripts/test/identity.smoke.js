#!/usr/bin/env node
// The plugin is onedash-1-22 and nothing in it still points at the social pipeline's root
...
assert.strictEqual(plugin.name, 'onedash-1-22', 'plugin.json name');
assert.ok(/1-22/.test(plugin.displayName || ''), 'displayName names 1-22');
const market = readJson('.claude-plugin/marketplace.json');
assert.ok(market.plugins.some(p => p.name === 'onedash-1-22'), 'marketplace lists onedash-1-22');
assert.ok(!market.plugins.some(p => p.name === 'social-pipeline'), 'marketplace no longer lists social-pipeline');

const ws = require('../lib-workspace.js');
assert.strictEqual(ws.CONFIG_DIR, '.onedash-1-22', 'config dir');
...
console.log('ok   plugin identity is onedash-1-22 and no social root or config dir remains');
